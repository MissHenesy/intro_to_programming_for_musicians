describe(`Testing Assignment 8.1, 'MySet Revisited'.
  ...
  it ("should compare two sets and tell if they are equal to each other", () => {
    test_set.add("Chico").add("Harpo").add("Groucho");
    compare_set.add("Chico").add("Harpo").add("Groucho");
    expect(test_set.is_equal(compare_set)).toBe(true);

    compare_set.add("Gummo").add("Zeppo");
    expect(test_set.is_equal(compare_set)).toBe(false);
  });
  ...
  it ("should receive duplicate items, but only contain unique items", () => {
    compare_set = new MySet([1,2,3]);
  ...
  it ("should call a forEach function and not blow up", () => {
    test_set.add("tea").add("honey").add("milk").add("scones");
